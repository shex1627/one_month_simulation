...
class Do_hw(Activity):
    ...
    def hard_question(self):
        if Do_hw.RANDOM_FACTOR.toss_bool(0):
            ...
            check_wp = check_float_input(50) #50% chance spawning hard questions
            ...
            if user_input == '0':
                ...
                if self.performer.spend_wp(1):
                    progress = 1 * Do_hw.HARD_QUESTION_RATE #need to spend 1 extra wp
                    self.performer.boost_hw(progress)
                    return progress
                else:
                    return 0
...
class Person:
    # the person class for the main character
        ...
        def stat(self):
            print("competitiveness: {0} \
                 \nhealth index   : {1} \
                 \nsleep  quality : {2}"
                  .format(self._competitiveness,
                          self._health,
                          self._sleep_quality))
...
def main():
        ...
        print("\nif this is your first time playing, \
               \nuse command 'help' to find out other commands.")
